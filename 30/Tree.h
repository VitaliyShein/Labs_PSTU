...
void Tree<T>::insertNode(T dt)
{
	...
	if (search)
	{
		cout << "Такой элемент уже есть в дереве\n";
	}
}
...
void Tree<T>::RootColPath(T k)
{
	...
	if (e)
	{
		...
		cout <<"Количество рёбер от корня до заданного графа равно " << col << endl;
	}
	else
	{
		cout << "Не было найдено графа с таким параметром" << endl;
	}
}
...
void drawButtons()
{
	...
	glEnd();
	glColor3fv(TextButtons);
	drawText1("Find path to the root", shift, height - shift - 80);
	...
	glEnd();
	glColor3fv(TextButtons);
	drawText1("Find max", shift, height - shift - 120);
}
